...
final class KeyValidatorTest extends TestCase
{
    ...
    public function it_fails_with_invalid_private_key(): void
    {
        ...
        } catch (UnprocessableKey $e) {
            self::assertEquals('Unable to read private key-data, invalid key provided?', $e->getMessage());
            self::assertNotEmpty($e->getPrevious()?->getMessage());

            self::assertTranslationEquals('Unable to process private key. Only PEM encoded X.509 files are supported.', $e);
        }
    }
    ...
    public function it_fails_with_key_mismatch(): void
    {
        ...
        try {
            ...
            self::fail('Exception was expected.');
        } catch (PrivateKeyMismatch $e) {
            self::assertTranslationEquals('The certificate does not match with the provided private-key.', $e);
        }
    }
    ...
    #[Test]
    #[DoesNotPerformAssertions]
    public function it_accepts_hidden_string_data(): void
    {
        $validator = new KeyValidator();
        $validator->validate(
            new HiddenString(
    ...
    public function it_fails_with_low_bit_count(): void
    {
        ...
        try {
            ...
            self::fail('Exception was expected.');
        } catch (KeyBitsTooLow $e) {
            self::assertSame(['expected' => 2048, 'provided' => 1024], $e->getParameters());

            self::assertTranslationEquals('The private-key bits-size 1024 is too low. Expected at least 2048 bits.', $e);
        }
    }
}
